...
import { ICreateDrinkPayload, IPutDrinkPayload, IUpdateDrinkPayload } from '../dto/drink-dto'
...
class DrinkService implements IDrinkService {
  ...
  async LikeDrink(id: number) {
    try {
      ...
      const updatedDrink = await this._repo.UpdateDrink({ likes: drink.likes + 1 }, { id })
